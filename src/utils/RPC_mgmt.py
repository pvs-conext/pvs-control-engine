...
class RPC_mgmt():
    ...
    def process_streamChannel(self, request_iterator, context):
		...
		print req
            ...
            # Process an "other" message that is not for authentication.
            #elif req.HasField("other"):
    ...
    def ProcessPacketOut(packet_out):
        ...
        switch_id = struct.pack("B", metadata.metadata_id)
        input_port = metadata.value
        payload = packet_out.payload

        # Calling the method to send the packet to the network interface.
        RPC_mgmt.SendPacket(switch_id, input_port, payload, ServerConfig.PACKETOUT_IFACE)
        print "Packet-out: server => interface %.9f" % time.time()

    @staticmethod
    def SendPacket(switch_id, switch_port, payload, interface):
        sock = socket.socket(socket.AF_PACKET, socket.SOCK_RAW)
        sock.bind((interface, 0))
        return sock.send(switch_id + switch_port + payload)
		...
		if check_bytestr:
			...
			match_key = ast.literal_eval(json.dumps(match_key))
                ...
                if request_update.type == p4runtime_pb2.Update.INSERT:
                    ServerConfig.print_debug("Processing insert request from peer {}({}), table {}, switch {}...".format(peer_key, username, switch_table.table_name, switch.switch_name))
                ...
                # TODO: refactor.
                    ...
                    ServerConfig.print_debug("Processing modify request from peer {}({}), table {}, switch {}...".format(peer_key, username, switch_table.table_name, switch.switch_name))
                ...
                # TODO: refactor.
                    ...
                    ServerConfig.print_debug("Processing delete request from peer {}({}), table {}, switch {}...".format(peer_key, username, switch_table.table_name, switch.switch_name))
                    ...
                    ServerConfig.print_debug("Error processing table write request from peer {}({}): update code {} is invalid".format(peer_key, username, update[0].type))
                    ...
                    ServerConfig.print_debug("Response sent to peer {}({})".format(peer_key, username))
                    return response
            ...
            # TODO: refactor.
                ...
                if ConnectionArray.verifyPermission(context, switch, PermEnum.DEVICE_WRITE | PermEnum.RESOURCE_WRITE) is False:
                    ...
                    ServerConfig.print_debug("Response sent to peer '{}' ({})".format(context.peer(), ConnectionArray.getUsername(context.peer())))
                    return response

                register_id = request_update.entity.register_entry.register_id
                register = SwitchConf.getRegisterById(switch_id, register_id)
                ServerConfig.print_debug("Processing write request from peer '{}' ({}), register '{}', switch id {} - Value: {}...".format(context.peer(), ConnectionArray.getUsername(context.peer()), register.reg_name, register.switch_id, int(request_update.entity.register_entry.data.enum_value)))

                # print "REGISTER INFORMATION IN RETRIEVED OBJECT:"
                # print "Object: {}".format(register)
                # print "Switch id: {}".format(register.switch_id)
                # print "Register id: {}".format(register.reg_id)
                # print "Register name: {}".format(register.reg_name)
    ...
    def process_table_read(peer_key, username, switch, entry):
        ...
        # print "\nTABLE INFO:"
        # print "Object: {}".format(switch_table)
        # print "Switch id: {}".format(switch_table.switch_id)
        # print "Table id: {}".format(switch_table.table_id)
        # print "Table name: {}".format(switch_table.table_name)
        # print "Match type: {}".format(switch_table.match_type)
        # print "Base address: {}\n".format(switch_table.base_address)
        ...
        # print "Found type: {}".format(type(found))
        # print "Found value: {}".format(found)
        ...
        resp_entity.table_entry.table_id = table.table_id + 10
        ...
        # TODO: continue this.
        # entity.table_entry.action.action.action_id = None
        # ...
    ...
    def process_register_read(switch_id, entry):
        ...
        # print "Register data type: {}".format(type(data))
        # print "Register data: {}\n".format(data)
    ...
    def process_counter_read(switch_id, entry):
        ...
        # print "Switch id: {}".format(counter.switch_id)
        # print "Counter id: {}".format(counter.reg_id)
        # print "Counter name: {}".format(counter.reg_name)
        ...
        # print "Counter data type: {}".format(type(data))
        # print "Counter data: {}\n".format(data)
