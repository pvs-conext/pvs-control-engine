...
class SwitchConfDAO():
    ...
    def addSwitch(dict, switch_id, switch_name, dir_path):
        try:
            ...
            try:
                ...
                dbVar.commit()
                dict[(switch_id)] = SwitchObject(switch_id, switch_name, dir_path)
                return True
            except:
                return False
        finally:
    ...
    def getSwitchByName(switch_name):
        try:
            lock.acquire(True)
            queryCursor.execute("SELECT device_id, switch_name, dir_path FROM switches WHERE switch_name=?", (switch_name,))
            row = queryCursor.fetchone()
            if row is None:
                return False
            else:
    ...
    def getSwitchById(switch_id):
        try:
            lock.acquire(True)
            queryCursor.execute("SELECT device_id, switch_name, dir_path FROM switches WHERE device_id=?", (switch_id,))
            row = queryCursor.fetchone()
            if row is None:
                return False
            else:
