...
FORWARD = 1
forward_table_dict = {
    "10.0.0.1": "00:15:4d:13:61:49 0b00000001",
    "10.1.0.1": "00:15:4d:13:61:4a 0b00010000",
    "10.0.0.2": "00:15:4d:13:61:07 0b00000100",
    "10.1.0.2": "00:15:4d:13:61:08 0b01000000",
}

SEND_FRAME = 2
send_frame_dict = {
    "0b00000001 ": "05:11:11:11:11:05",
    "0b00000100 ": "05:22:22:22:22:05",
    "0b00010000 ": "05:33:33:33:33:05",
    "0b01000000 ": "05:44:44:44:44:05"
}
...
Note that the 'router' switch currently has id 2.
"""
    ...
    while True:
        try:
            ...
            if packet.HasField("packet") and packet.packet.metadata[0].metadata_id == dev_id:
                print "Packet-in: server => router controller %.9f" % time.time()
		...
		# The packet can be either ipv4 or 802.1q (vlan).
                if ether_type == 2048 or ether_type == 33024:
                    ...
                    client.WriteTableEntry(p4runtime_pb2.Update.INSERT, IPV4_NHOP, ip_dst, ipv4_nhop_dict[ip_dst])
                    ...
                    print "Packet-out: router controller => server %.9f" % time.time()

            if packet.HasField("other"):
                if tables_initialized is False:
                    for ip in send_frame_dict:
                        client.WriteTableEntry(p4runtime_pb2.Update.INSERT, SEND_FRAME, ip, send_frame_dict[ip])
